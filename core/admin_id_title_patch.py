...
def apply_member_id_and_facturatie():
    ...
    has_federale = any(
        getattr(f, "name", None) == "federale_bijdrage_via_spiegelven"
    ...
    def new_get_fieldsets(self, request, obj=None, *args, **kwargs):
        ...
        for title, cfg in fs:
            ...
            if tl in {"facturatie", "facturering", "billing"}:
                ...
                if has_federale and "federale_bijdrage_via_spiegelven" not in fields:
                    fields.append("federale_bijdrage_via_spiegelven")
